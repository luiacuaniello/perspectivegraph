...
| B1 metrics | I | **`GET /metrics` is open by design** - unauthenticated and unthrottled, so a scrape never starves - but it sits on the same mux and port as the API, and several series carry a `tenant` label: `analyzer_critical_paths`, `analyzer_graph_nodes`, `analyzer_graph_edges`. Anyone who can reach the port therefore enumerates tenant names and learns how large each estate is and how many critical paths it currently has | Path contents, asset names and scores are never exposed - only counts and timings | Aggregate but real: "which tenants exist, and which has the worst posture right now" is exactly the shape of a targeting signal. Do not expose the API port directly; scrape from inside the cluster and publish only what the gateway allows. If your deployment is multi-tenant and the port is reachable, treat this as a leak to close at the gateway, since the endpoint itself has no auth switch |
