...
// socket, writing to a sensitive path. AegisGraph turns each alert into a
...
// decode accepts a single alert, a JSON array, or NDJSON.
func decode(r io.Reader) ([]alert, error) {
	...
	// Single object or NDJSON: decode line by line (also handles one object).
	var out []alert
	...
	for sc.Scan() {
		...
		if err := json.Unmarshal(line, &a); err != nil {
			return nil, fmt.Errorf("decode falco line: %w", err)
		}
		out = append(out, a)
	}
	return out, sc.Err()
