...
type Service struct {
	...
	calibrator func(tenant string) (brier, ece float64, samples int)
...
// WithCalibrator injects a function returning a tenant's current calibration headline
// (Brier/ECE/sample-count), so each pass samples the calibration trend into history -
// letting an operator watch the evidence accumulate. Nil disables it. Returns the
// service for chaining.
func (s *Service) WithCalibrator(f func(tenant string) (brier, ece float64, samples int)) *Service {
	s.calibrator = f
	return s
}
...
func (s *Service) runTenant(ctx context.Context, tenant string) {
	...
	// Calibration trend: sample each pass (verdicts change independently of the graph,
	// so record it before change-detection can skip the rest of the work).
	if s.calibrator != nil && s.history != nil {
		if b, e, n := s.calibrator(tenant); n > 0 {
			s.history.SampleCalibration(tenant, b, e, n)
		}
	}
