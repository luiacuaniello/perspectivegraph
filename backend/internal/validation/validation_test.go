...
func TestPutValidation(t *testing.T) {
	...
	if _, err := s.Put(context.Background(), Record{Outcome: Missed, Source: "redteam", Route: "internet → saas → db"}); err != nil {
...
func TestLatestVerdictPerPathWins(t *testing.T) {
	s, _ := New("")
	_, _ = s.Put(context.Background(), Record{Outcome: Refuted, Source: "bas", PathID: "ap-1"})
	_, _ = s.Put(context.Background(), Record{Outcome: Confirmed, Source: "redteam", PathID: "ap-1"}) // retest
	...
	if m, _ := s.Metrics(context.Background(), "default"); m.Confirmed != 1 || m.Refuted != 0 {
...
func TestPrecisionRecall(t *testing.T) {
	...
	for _, id := range []string{"ap-1", "ap-2", "ap-3"} {
		_, _ = s.Put(context.Background(), Record{Outcome: Confirmed, Source: "caldera", PathID: id})
	}
	_, _ = s.Put(context.Background(), Record{Outcome: Refuted, Source: "caldera", PathID: "ap-4"})
	_, _ = s.Put(context.Background(), Record{Outcome: Partial, Source: "caldera", PathID: "ap-5"})
	_, _ = s.Put(context.Background(), Record{Outcome: Missed, Source: "redteam", Route: "okta → admin"})

	m, _ := s.Metrics(context.Background(), "default")
...
func TestEmptyMetricsUndefined(t *testing.T) {
	s, _ := New("")
	if m, _ := s.Metrics(context.Background(), "default"); m.HasData {
...
func TestPersistenceRoundtrip(t *testing.T) {
	...
	_, _ = s.Put(context.Background(), Record{Outcome: Confirmed, Source: "redteam", PathID: "ap-9", Tenant: "globex", Evidence: "caldera run #42"})

	s2, _ := New(path)
	rec, ok, _ := s2.Get(context.Background(), "globex", "ap-9")
...
func TestNilStoreIsNoop(t *testing.T) {
	var s *Store
	if l, _ := s.List(context.Background(), "default"); l != nil || s.Persistent() {
