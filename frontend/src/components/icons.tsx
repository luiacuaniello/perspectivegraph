...
export const ScissorsIcon = make(
  <>
...
const CloudIcon = make(<path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z" />);
...
const ASSET_ICONS: Record<string, (props: IconProps) => ReactElement> = {
  ...
  Library: BookIcon,
  User: UserIcon,
  IAM_Role: UserCheckIcon,
  ServiceAccount: CpuIcon,
  CVE: BugIcon,
  Weakness: AlertTriangleIcon,
  Misconfiguration: WrenchIcon,
  Secret: KeyIcon,
};

export function AssetIcon({ label, ...props }: { label: string } & IconProps) {
  const Icon = ASSET_ICONS[label] ?? BoxIcon;
  return <Icon {...props} />;
}
