...
- **Monte Carlo risk quantification.** Each trial realizes every edge independently (present
  ...
  This is the `P(at least one crown jewel compromised)` a CISO actually asks for. It is *also*
...
| Connectors (pull) | `internal/connector` | Agentless, scheduled PULL sources that feed the **same** bus, so the whole downstream pipeline is reused. A leader-only `Scheduler` (mirrors `analyzer.Service`) polls each `Connector`, isolates per-source failures, and exposes health at `GET /connectors` + `connector_*` metrics. Connector `aws` reuses the `cloudnet`/`iam` collectors verbatim (transport-abstracted: `fixtures` for demo/test, `aws-sdk-go-v2` for live); `azure` maps normalized `az` network state (NSG rules, VMs, VNet peerings) onto the `cloudnet` shape, then reuses that collector (transport-abstracted: `fixtures`, with `azure-sdk-for-go` the wired extension point) |
...
- [x] Headline/score coherence (the correlation model now matches end to end): the Monte Carlo headline is marginalized over the attacker-profile mixture (`mixtureCompromiseProbability = Σ_c P(c)·R_c`; `profileCompromise` breakdown), reusing the same `conditionalProb(p,basis,skill)` as the per-path mixture. The independent `anyCompromiseProbability` stays as the baseline; the criminal profile (skill 0) reproduces it exactly, anchoring the mixture. Closes the gap where the per-path score was correlation-aware but the headline reliability silently assumed independent edges
- [x] **#6 decision tool** (`andprobe` subcommand + `make and-probe`): before building a Bayesian Attack Graph, answer the question that decides it - does the environment have real **AND semantics** (a compromise needing several distinct prerequisites at once) or is it pure OR-reachability the Monte Carlo already models? Scans the live graph and counts critical-path nodes whose incoming edges span >= 2 prerequisite categories (network/vulnerability/identity/escape). Explicitly an *upper-bound heuristic* (a pure graph can't tell AND from OR - that's what a BAG adds), so it names candidates for a human/verdict to confirm; the ground truth is real `refuted` verdicts where a missing precondition = a true AND node. Near-zero ⇒ #6 is a no-op, invest in better `p(e)` instead. `--all-nodes` inspects the whole topology, not just attack-path nodes. Because AND-semantics is a *topology* property (not a CVE one), feed it real topology: `make ingest-k8s` snapshots the current `kubectl` context (the k8s collector takes native `kubectl get ingress,service,pod,sa,rbac -A -o json` → exposure/privesc/escape edges) → point it at a local `kind + kubernetes-goat` cluster for a vulnerable one. Tried live against a real Docker Desktop cluster (145 nodes/89 edges ingested): a locked-down cluster yields zero AND candidates - itself the answer
...
- [x] Transport & supply-chain hardening pass: explicit `http.Server` timeouts (ReadHeader/Read/Write/Idle - Slowloris defense), dedicated timeout-bound HTTP clients + size-capped response reads (`httpx`, JWKS), CORS **allowlist** (`CORS_ALLOWED_ORIGINS`) replacing `*`, **fail-closed OIDC** (refuse to start if `OIDC_JWKS_URL` is set without `iss`/`aud`), **opt-in TLS that isn't hardcoded off** - the API + ingest servers serve HTTPS (TLS ≥ 1.2) directly when `TLS_CERT_FILE`/`TLS_KEY_FILE` are set (else plain HTTP for a proxy/ingress to terminate); the Helm chart adds `ingress.tls` (cert-manager-friendly) and in-pod `backend.tls`; the **DB sslmode is configurable** (`POSTGRES_SSLMODE`, no longer a hardcoded `disable` - the chart defaults an external Postgres to `require`, demo to `disable`, and `verify-full` is one flag away), **NATS TLS/mTLS** via `NATS_TLS_CA`/`NATS_TLS_CERT`/`NATS_TLS_KEY` (or a service mesh that wraps all in-cluster traffic transparently), and the chart's credential Secret can be **brought from outside** (`secrets.existingSecret` for External Secrets / Sealed Secrets / Vault), and a **SAST gate (`gosec`) + secret scan (`gitleaks`) in CI** - the tool's own source held to the bar it sets (the few benign findings carry justified `// #nosec`). Test coverage closed on the two highest-risk untested packages: the **Trivy parser** (table test + a `FuzzParse` contract - untrusted webhook bytes must never panic and never emit a malformed node/edge) and **leader election** (fail-closed unit tests + a Postgres advisory-lock mutual-exclusion/failover integration test on the CI AGE service)
...
- [x] Container-escape vector (`ESCAPES_TO`): the K8s collector flags pods that break the host boundary (privileged container, **a dangerous added Linux capability** - `SYS_ADMIN`/`SYS_MODULE`/`SYS_PTRACE`/`SYS_RAWIO`/`SYS_BOOT`/`DAC_READ_SEARCH`/`DAC_OVERRIDE`/`BPF`, or `ALL` - privileged-equivalent per CISA/NSA even without `privileged:true`, `hostPath` mount, `hostPID`/`hostNetwork`/`hostIPC`) and emits an escape edge to the synthetic cluster-admin (ATT&CK **T1611 Escape to Host**) - so *internet → privileged pod → host → cluster takeover* becomes a first-class, ranked attack path, distinct from RBAC privilege escalation
- [x] Group/User RBAC subjects (`ASSUMES` from group bindings): a `(Cluster)RoleBinding` whose subject is a **Group** or **User** - not just a `ServiceAccount` - now grants the role to the workloads it covers, closing a real false-negative (a real `missed` verdict caught it on a live cluster). Kubernetes' built-in serviceaccount groups expand to concrete pods: `system:serviceaccounts:<ns>` to that namespace's SAs, `system:serviceaccounts`/`system:authenticated` to every SA; `system:unauthenticated`/`system:anonymous` binds the role straight to an **internet-exposed anonymous principal** (no credentials needed); a named/OIDC group or user (no cluster-visible membership) is recorded as a standalone principal so the grant stays visible. So *internet → pod → SA (via a group binding) → cluster-admin* is now surfaced, where the ServiceAccount-only view drew no edge at all
- [x] Agentless connectors (`internal/connector` + `internal/connector/aws` + `internal/connector/azure`): scheduled, leader-only PULL sources that reach out to a cloud account instead of waiting for a report upload - the first step toward "connect read-only, see attack paths in 15 minutes". They publish onto the same bus, so identity resolution / graph / analyzer are reused unchanged. The AWS connector reuses the existing `cloudnet`/`iam` collectors verbatim behind a swappable transport: `fixtures` (local JSON - proves the pull end-to-end with no credentials) and **`sdk`** (live AWS via `aws-sdk-go-v2`: EC2 `describe-security-groups`/`describe-instances`/`describe-vpc-peering-connections` → cloudnet JSON, `GetAccountAuthorizationDetails` → iam JSON, both paginated; standard credential chain + optional cross-account read-only `AssumeRole`; the EC2/IAM clients sit behind a narrow `ec2API`/`iamAPI` interface so the mapping is unit-tested with a fake - no account needed). The Azure connector adds a thin mapping layer (Azure's native model differs from the collector shape): NSG inbound Allow rules → security groups, VMs → instances (bound to their NSGs + tags), VNet peerings → VPC peerings - Azure's `Internet` service tag normalizes to `0.0.0.0/0` so exposure is detected - then the **same** `cloudnet` collector parses the result. Its transport is `fixtures` today (normalized `az network`/`az vm -o json` state, no credentials), with `azure-sdk-for-go` the wired extension point. Config `CONNECTORS_ENABLED` / `CONNECTOR_INTERVAL` / `AWS_CONNECTOR_MODE` (`fixtures`|`sdk`) / `AWS_ROLE_ARN` / `AZURE_CONNECTOR_MODE`; health at `GET /connectors`; Prometheus `connector_runs_total` / `connector_events_total`. Verified: a fixtures pull populated an isolated graph (9 nodes → 2 attack paths) with no file upload; the AWS SDK→JSON mapping (incl. an internet-exposed instance from a `0.0.0.0/0` SG) is covered by a fake-client test; the Azure mapping (Internet-open NSG → internet-exposed VM, PII VM as crown jewel) by a fixtures test
...
- [x] Robustness pass: Monte Carlo input-uncertainty band (now a principled per-edge **Beta-posterior credible band** - concentration scaled by weight-basis confidence - replacing the old flat ±30% scaling; shown as "modeled X-Y%"), plus a **per-path 90% credible interval** on the score (`scoreCiLow`/`scoreCiHigh`, propagating the same posteriors through the product; `uncertainty.go`, dependency-free Marsaglia-Tsang sampler); leader-elected side-effects so multiple replicas don't duplicate drift/PR actions; Vitest frontend suite + CI; React error boundary; "analyzed N ago" freshness
- [x] Attacker-profile mixture (independence fix at the root): the score is marginalized over a latent attacker-capability variable - `S(P) = Σ P(c)·∏ p(e|c)` over commodity/criminal/apt profiles (`profiles.go`). Conditioning makes the product honest within a profile; marginalizing reintroduces the positive correlation `∏p` drops. Surfaces `mixtureScore` + a per-profile `profileScores` breakdown ("72% vs an APT, 18% vs commodity") on each path; priors retunable via `ATTACKER_PROFILE_PRIORS` (config → compose → Helm). The naive `Score` stays as the independent baseline
- [x] **One coherent posterior (P2)**: the per-path uncertainty used to be four numbers describing *different* quantities that didn't nest - the point `score` (`∏p`), the `scoreUpperBound` (Fréchet ceiling), a credible interval computed on the *independent product*, and a separate `mixtureScore`. `unifiedScorePosterior` (`uncertainty.go`) replaces the interval with a single Monte Carlo that composes **epistemic** uncertainty (each hop's Beta posterior, sampled once per draw so the same uncertain world faces every attacker) with the **attacker-capability mixture** (`Σ_c P(c)·∏ p(e|c)` at the sampled probabilities). It exposes `posteriorMean` - the coherent point estimate the credible interval now brackets, which even corrects the Jensen gap the plug-in `mixtureScore` ignores - with `[scoreCiLow, scoreCiHigh]` around it. Being the attacker-marginal, `posteriorMean` can sit above or below the bare `score`; it stays under `scoreUpperBound` and inside its interval. Beta concentration κ is **evidence-count-derived** when an edge carries an `evidence_count` (`κ = count + prior`, the proper Bayesian width), heuristic otherwise. Deterministic (seeded per path id) so parallel pathfinding stays byte-identical. (Collectors emitting `evidence_count` is the remaining data task; the model is ready for it.)
- [x] **Common-cause correlation in the risk Monte Carlo (P3)**: the headline compromise probability sampled every edge independently, so two routes that both rest on the *same* weakness (one CVE, one leaked credential) counted as genuine redundancy and overstated reachability - the correlation `scoreUpperBound` bounds per-path but the aggregate simulation ignored. Edges can now carry a `weight_cause` (the shared CVE/credential id); the Monte Carlo couples same-cause edges **comonotonically** - one uniform draw per cause per trial, so a shared weakness holds or fails as a unit and takes all its routes with it (`P(all edges of a cause) = min p`, the Fréchet coupling). Refactored the three reachability passes (point, epistemic credible band, attacker-mixture) onto one `reachTrial(seeds, adj, present)` with a `comonotonicPresent` closure, so all three share the coupling; deterministic and a no-op for causeless edges (the demo is unchanged). `montecarlo.go`. Collectors stamping `weight_cause` is the remaining data task; the model is ready
- [x] **EPSS conditional-traversal map (P4, opt-in)**: EPSS is a *marginal* 30-day exploitation-in-the-wild rate, not the conditional a positioned attacker's traversal needs (`P(traverse this edge | already on the path)`), so using it as `p(e)` understates a present attacker - the deepest input-provenance gap. `threatintel.EdgeProbability` now routes EPSS through an explicit `TraversalFromEPSS(epss) = epss^gamma` hook: `gamma < 1` lifts the marginal toward the conditional, `gamma = 1` (default) keeps EPSS as-is (the honest baseline the calibration loop grades). Configurable via `EPSS_TRAVERSAL_GAMMA` (config -> compose -> Helm); the KEV branch (observed exploitation) is untouched. It is a documented **prior**, not a fitted map - fitting `p(traverse|EPSS)` needs per-edge verdict ground truth (path-level verdicts don't isolate one edge), so the engine never rewrites the input silently; the operator opts in. Closes the math-review loop: the five soundness gaps are now fixed or model-ready
...
- [x] Minimal design pass: flat surfaces (dropped glass blur, tactical grid mesh, accent glows, neon text glow), one indigo accent with semantic red/amber reserved for real risk, sentence case throughout, and a hierarchy-first overview (one hero metric - account compromise, with its calibration verdict - over a compact secondary strip, replacing nine co-equal rainbow cards). Driven from the design tokens (`index.css`, `tailwind.config.js`) so the whole app re-themes together; dark default, refined light
...
- [x] Probability **calibration** (the demo→production gate): every verdict captures the path's predicted score `S(P)` server-side, so the verdict log becomes a calibration dataset. Reports **Brier**, **log loss**, **ECE** and a **reliability diagram** (predicted vs observed per bucket) with an honest verdict (well-calibrated / over- / under-confident) and an *advisory* rescale (`observed/predicted`, surfaced not auto-applied). Exposed as the GraphQL `calibration` query + in `GET /validations`; rendered as a Calibration panel on the Overview - the evidence that lets you defend the scores *as probabilities*, not labels
- [x] Calibration **scoped to the event it grades** (a correctness fix over a single conflated track): a verdict carries a `scope`. A **path-scoped** verdict validated one specific surfaced path, so it grades that path's `S(P)` against "this path was traversable"; a **target-scoped** verdict validated whether a crown jewel was reached *at all, by any route* - what a BAS "I popped the jewel" run actually reports - so it grades the **per-target compromise probability** (the Monte Carlo `CrownJewelRisk`, captured server-side) against "the jewel was reached". Grading `S(P)` against an any-route outcome conflates two different events and biases the report, so the two run as **independent tracks**: the top-level `calibration` is path-scoped, the nested `calibration.target` is target-scoped. `POST /validations` accepts `scope: path|target` (default `path`, back-compatible); `importverdicts` passes it through so a BAS target-reached finding is graded correctly. A path may carry one of each (they validate different events)
- [x] Calibration **diagnostics** (the instrument that decides #6 vs #7 instead of building blind): **isotonic recalibration** (`brierRecalibrated` = the floor a rescale reaches, + a `recalibrationMap` consumers apply out-of-band, never a silent rewrite); **segmented** calibration by path structure (correlated/independent, long/short - residual on correlated/long ⇒ structural #6); a **detection axis** (operator marks a confirmed verdict `detected`; high catch rate on high-score paths ⇒ #7); resolution measured via the binned Murphy term (so a constant forecast is flagged low-resolution despite being "calibrated"); all folded into one `diagnosis` (`recalibrate-first | structural (#6) | detection-axis (#7) | low-resolution`). `internal/validation/diagnostics.go`; shown in the Calibration panel
- [x] **Per-basis recalibration (P1)**: the global isotonic map is monotone in the raw score, so it cannot fix a bias *structured by evidence provenance* - if EPSS-derived hops run hot and heuristic hops run cold at the *same* score, no single curve corrects both, and the two can cancel at the aggregate so the global report reads "well-calibrated" while the model is badly miscalibrated per class. The engine buckets each verdict by the path's weakest-evidence basis (kev/epss/cvss/severity/heuristic) and fits a per-basis **Platt** correction `sigmoid(a + b·logit(score))`, ridge-shrunk toward the identity so a thin bucket stays near the raw score; the Brier is cross-validated. `brierRecalibratedByBasis` vs the global `brierRecalibrated` measures how provenance-structured the miscalibration is; `basisSegments` attributes it; `recalibrationByBasis` is the per-basis map; the `diagnosis` recommends it (before low-resolution/#6, and it fires only with >=2 distinct bases). A `per-basis` self-test scenario + CI test guard it. This directly attacks the "EPSS is a marginal rate, not a conditional per-edge probability" incommensurability at the point where data can correct it
- [x] Calibration **self-test** (`genverdicts` subcommand + `make calibration-selftest SCENARIO=...`): a ground-truth simulator that draws verdicts from a *known* reality (calibrated / overconfident / underconfident / correlated / low-resolution / detection) and checks the diagnostics name the right cause - validates the *instrument* (and seeds the dashboard) with no real vulnerable infra. POST `/validations` accepts client-supplied calibration features (`predictedScore`/`hops`/`correlatedHops`) as a fallback for such offline verdicts; for a live path the server-captured values always win. Building this found two real bugs in `diagnose` (an absolute-Brier threshold that ignored irreducible Bernoulli variance; checking calibration before resolution) - the point of a self-test
- [x] Calibration as a **program, not a snapshot** (production-readiness for the real-verdict phase): the recalibrated Brier is now **k-fold cross-validated** (out-of-sample, so it doesn't overfit on thin real data); the report carries `persistent` and the UI flags an `in-memory` dataset (set `VALIDATIONS_PATH`, or restart-loss bites a real engagement); a **calibration trend** (`history.CalibrationSample`, sampled each analyzer pass via an injected calibrator, coalesced 1/min, persisted) exposes Brier/ECE/samples over time as `calibrationTrend` - shown as a "Brier over time" sparkline so an operator can watch the evidence accumulate and the scores improve
- [x] **Verdict flywheel** (the prerequisite for calibrating on real data): the verdict store is durable (file-backed + encrypted at rest via `VALIDATIONS_PATH`); the Helm chart persists it on a PVC with a pod `fsGroup` so the nonroot backend can actually write it (previously `persistence.enabled` mounted a root-owned volume and silently failed the first write). Automatic ingestion via `POST /validations/import`: a BAS platform's post-run webhook posts a whole report and the server matches each finding to a live path (by path id, or crown-jewel target + entry) and records it with the right `scope`, so verdicts flow in without a CLI cron - turning "you can import verdicts" into "verdicts accumulate on their own"
- [x] Pre-flight before real targets: (a) **input-provenance review** - the EPSS semantic gap (a *marginal* exploitation rate, not a conditional per-edge traversal probability) and the heuristic `severity → p` anchors are documented at the source (`internal/threatintel`, `internal/ingestion/severity.go`), fed as-is on purpose so calibration reveals/corrects the bias on real verdicts rather than a silent transform; (b) **diagnostics CI guard** - the self-test scenarios run in-process and deterministically (`validation.GenerateScenario` shared by the `genverdicts` CLI and `TestCalibrationScenarioDiagnosesEndToEnd`), so a regression in the gate fails the build
- [x] Real-verdict bridge (`importverdicts` subcommand + `make import-verdicts`): a **tool-agnostic** BAS/red-team report (Pacu, Caldera, a manual pentest) is matched onto the engine's live paths by target crown-jewel name and optional entry (or an explicit path id), so a real run feeds precision/recall + calibration + the detection axis with no custom integration - `outcome`∈confirmed/refuted/partial/missed, plus `detected` for #7. Closes the loop on reality: vulnerable target (CloudGoat via the AWS connector `fixtures`/`sdk`, or a local Juice Shop via a scanner) → live paths → BAS → `import-verdicts` → calibration; `VALIDATIONS_PATH` persists a real engagement. A zero-cost on-ramp is baked in: `ingestreal` (`make ingest-real IMAGE=…`) Trivy-scans a genuinely vulnerable image (real CVE/CVSS; real KEV/EPSS with `THREATINTEL=on`) and wires the minimal topology so the CVE sits on an internet→crown-jewel path - e.g. a local vulhub log4shell → `internet-lb → image → log4j-core → CVE-2021-44228 → crown-jewel`, a path you can actually exploit to generate a real verdict
